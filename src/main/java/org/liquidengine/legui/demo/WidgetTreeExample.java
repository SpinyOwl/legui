...
import static org.liquidengine.legui.style.color.ColorUtil.half;
import static org.liquidengine.legui.style.color.ColorUtil.randomColor;
...
public class WidgetTreeExample extends Demo {
    ...
    private Widget createFlexWidget(String name) {
        ...
        widget.getContainer().getStyle().getFlexStyle().setFlexDirection(FlexStyle.FlexDirection.COLUMN);
        return widget;
    }
    ...
    private void setChildStylesAndCallbacks(Widget root, Widget widget, boolean withBottomMargin) {
        ...
        widget.getMinimizeButton().getListenerMap().addListener(MouseClickEvent.class, e -> {
            if (autoCloseWidgets && e.getAction() == MouseClickEvent.MouseClickAction.CLICK && !widget.isMinimized()) {
