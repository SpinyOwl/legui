...
import static org.liquidengine.legui.marshal.JsonConstants.SLIDER_ACTIVE_COLOR;
import static org.liquidengine.legui.marshal.JsonConstants.SLIDER_COLOR;
import static org.liquidengine.legui.marshal.JsonConstants.SLIDER_SIZE;
...
public class GsonSliderMarshaller<T extends Slider> extends GsonControllerMarshaller<T> {
    ...
    protected void unmarshal(JsonObject json, T object, GsonMarshalContext context) {
        ...
        if (isNotNull(sliderActiveColor)) {
            object.setSliderActiveColor(readColor(sliderActiveColor.getAsJsonObject()));
        }
        if (isNotNull(sliderColor)) {
            object.setSliderColor(readColor(sliderColor.getAsJsonObject()));
        }
        if (isNotNull(orientation)) {
            object.setOrientation(Orientation.valueOf(orientation.getAsString()));
        }
        if (isNotNull(sliderSize)) {
            object.setSliderSize(sliderSize.getAsFloat());
        }
        if (isNotNull(value)) {
