...
import static org.liquidengine.legui.marshal.JsonConstants.CHAR_CODE;
import static org.liquidengine.legui.marshal.JsonConstants.COLOR;
...
public class GsonCharIconMarshaller<I extends CharIcon> extends GsonIconMarshaller<I> {
    ...
    protected void unmarshal(JsonObject json, I object, GsonMarshalContext context) {
        ...
        if (isNotNull(color)) {
            object.setColor(GsonUtil.readColor(color.getAsJsonObject()));
        }
        if (isNotNull(charCode)) {
            object.setCharCode(charCode.getAsCharacter());
        }
