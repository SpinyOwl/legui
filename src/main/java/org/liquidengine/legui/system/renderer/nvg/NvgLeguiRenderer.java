...
public class NvgLeguiRenderer extends LeguiRenderer {
    ...
    protected NvgLeguiRendererProvider provider;

    public NvgLeguiRenderer(LeguiContext context, NvgLeguiRendererProvider provider) {
        super(context);
     ...
     */
    public static void renderBorder(Component component, LeguiContext context) {
        Border border = component.getBorder();
        if (border != null && border.isEnabled()) {
            // Render border
            LeguiBorderRenderer borderRenderer = NvgLeguiRendererProvider.getInstance().getBorderRenderer(border.getClass());
     ...
     */
    public static void renderBorderWScissor(Component component, LeguiContext context, long nanovg) {
     ...
     * @param context   context.
     */
    public static void renderIcon(Icon icon, Component component, LeguiContext context) {
        if (icon != null && component != null) {
            NvgLeguiRendererProvider.getInstance().getIconRenderer(icon.getClass()).render(icon, component, context);
        }
    }
     ...
     * @param context  context.
     */
    public static void renderImage(Image image, Vector2fc position, Vector2fc size, Map<String, Object> properties, LeguiContext context) {
        if (image != null) {
            NvgLeguiRendererProvider.getInstance().getImageRenderer(image.getClass()).render(image, position, size, properties, context);
        }
    }
    ...
    public void destroy() {
        ...
        provider.getComponentRenderers().forEach(LeguiComponentRenderer::destroy);
