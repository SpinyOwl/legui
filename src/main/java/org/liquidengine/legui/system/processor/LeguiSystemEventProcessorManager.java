 ...
 */
public class LeguiSystemEventProcessorManager {
    private final Frame        frame;
    private final LeguiContext context;
    private Queue<LeguiSystemEvent> eventQueue = new ConcurrentLinkedQueue<>();

    public LeguiSystemEventProcessorManager(Frame frame, LeguiContext context, LeguiCallbackKeeper keeper) {
        this.frame = frame;
        this.context = context;
        addCallbacks(keeper);
    }

    public LeguiSystemEventProcessorManager(Frame frame, LeguiContext context) {
        this.frame = frame;
        this.context = context;
    }
    ...
    public void processEvent() {
        for (LeguiSystemEvent event = eventQueue.poll(); event != null; event = eventQueue.poll()) {
            LeguiSystemEventProcessor processor = LeguiSystemEventProcessorProvider.getInstance().getProcessor(event.getClass());
    ...
    private void addCallbacks(LeguiCallbackKeeper guiCallbackKeeper) {
        //@formatter:off
