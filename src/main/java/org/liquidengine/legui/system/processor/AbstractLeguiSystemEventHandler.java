package org.liquidengine.legui.system.processor;

import org.liquidengine.legui.component.Frame;
import org.liquidengine.legui.component.Layer;
import org.liquidengine.legui.event.LeguiEvent;
import org.liquidengine.legui.system.context.LeguiContext;
import org.liquidengine.legui.system.event.LeguiSystemEvent;

import java.util.Collections;
import java.util.List;

/**
 * Abstract handler for {@link LeguiSystemEvent}s. Used to handle events and transform them to UI events (Subclasses of {@link LeguiEvent})
 */
public abstract class AbstractLeguiSystemEventHandler<E extends LeguiSystemEvent> implements LeguiSystemEventHandler<E> {

    /**
     * Default implementation of event handler {@link LeguiSystemEventHandler#handle(LeguiSystemEvent, Frame, LeguiContext)} method.
     * Used to handle events and check if event should be passed to underlying layer or not.
     *
     * @param event   event to handle.
     * @param frame   target frame for event.
     * @param context context.
     */
    public final void handle(E event, Frame frame, LeguiContext context) {
        preHandle(event, frame, context);
        List<Layer> layers = frame.getAllLayers();
        Collections.reverse(layers);
        for (Layer layer : layers) {
            if (layer.isEventReceivable()) {
                if (!layer.getContainer().isVisible() || !layer.getContainer().isEnabled()) continue;
                if (handle(event, layer, context)) {
                    return;
                }
            }
            if (!layer.isEventPassable()) return;
        }
        postHandle(event, frame, context);
    }

    /**
     * This method should be overrided to pre-handle some event.
     *
     * @param event   event which should be pre-processed.
     * @param frame   target frame for event.
     * @param context context
     */
    protected void preHandle(E event, Frame frame, LeguiContext context) {
        // This method should be overrided to pre-handle some event.
    }

    /**
     * This method used to handle some {@link LeguiSystemEvent}
     * and produce (or not) {@link LeguiEvent} instances (which are UI events)
     *
     * @param event   event to be processed.
     * @param layer   target event layer.
     * @param context context.
     * @return true if event processed and it shouldn't be processed for other underlying layers.
     */
    protected boolean handle(E event, Layer layer, LeguiContext context) {
        return false;
    }

    /**
     * This method should be overrided to post-handle some event.
     *
     * @param event   event which should be post-processed.
     * @param frame   target frame for event.
     * @param context context
     */
    protected void postHandle(E event, Frame frame, LeguiContext context) {
        // This method should be overrided to post-handle some event.
    }
}
