 ...
 */
public class SystemEventProcessorManager {
    private final Frame   frame;
    private final Context context;
    private Queue<SystemEvent> eventQueue = new ConcurrentLinkedQueue<>();

    public SystemEventProcessorManager(Frame frame, Context context, CallbackKeeper keeper) {
        this.frame = frame;
        this.context = context;
        addCallbacks(keeper);
    }

    public SystemEventProcessorManager(Frame frame, Context context) {
        this.frame = frame;
        this.context = context;
    }
    ...
    public void processEvent() {
        for (SystemEvent event = eventQueue.poll(); event != null; event = eventQueue.poll()) {
            SystemEventProcessor processor = SystemEventProcessorProvider.getInstance().getProcessor(event.getClass());
